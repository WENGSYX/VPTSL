...
class DebertaV2Model(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        if QGH_video != None:
            embedding_output = torch.cat([embedding_output, torch.stack([QGH_video])], axis=1)
            attention_mask = torch.cat([attention_mask, torch.tensor([[1]]).to(input_ids.device)], axis=1)
...
class VPTSLModel(DebertaV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config):
        ...
        self.deberta = DebertaV2Model(config)
        self.qa_outputs = nn.Linear(config.hidden_size, config.num_labels)
        self.video_affine = VisualProjection(visual_dim=1024, dim=1024, drop_rate=0.1)
        ...
        self.cq_attention1 = CQAttention(dim=1024, drop_rate=0.2)
        self.cq_concat = CQConcatenate(dim=1024)
        self.highlight_layer = HighLightLayer(dim=1024)
        self.QGH_linear = nn.Linear(1024, config.hidden_size)
    ...
    def forward(
        ...
        # audio_features = self.wav2vec2(audio_features).last_hidden_state

        outputs = self.deberta(
            input_ids[:, :input_ids.size(1) - sum(token_type_ids[0]).item()])

        query_features = self.query_linear(outputs.last_hidden_state.detach())
        # features = sequence_output * video_features[0].mean(1)
        v_mask = vfeats_mask
        # a_mask = torch.ones([1, audio_features.size(1)], device=video_features.device)
        features1 = self.cq_attention1(video_features, query_features, v_mask,
                                       attention_mask[:, :input_ids.size(1) - sum(token_type_ids[0]).item()])
        features = self.cq_concat(features1, query_features,
                                  attention_mask[:, :input_ids.size(1) - sum(token_type_ids[0]).item()])

        QGH_video = self.highlight_layer(features, v_mask)

        features = self.QGH_linear(QGH_video)
...
class VideoQuestionAnsweringModelOutput(ModelOutput):
    ...
    h_score: Optional[Tuple[torch.FloatTensor]] = None